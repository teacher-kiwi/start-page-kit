...
const Dashboard = () => {
  ...
  const loadClassroomData = async (teacherName: string) => {
    try {
      ...
      } = await supabase.from('classrooms').select('*').eq('teacher_name', teacherName).single();
      ...
      if (studentsError) {
        ...
        const studentInputsData = students.map((student, index) => ({
          ...
          name: student.name,
          image: null,
          // 이미지는 파일 객체가 아니므로 null로 설정
          imageUrl: student.photo_url || undefined,
          // 기존 이미지 URL
  ...
  useEffect(() => {
    ...
    const deriveName = (userObj: any) => {
      ...
      return meta.name || meta.full_name || meta.preferred_username || (userObj?.email ? userObj.email.split("@")[0] : "");
    };
  ...
  return <div className="min-h-screen bg-background p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-foreground">우리끼리 속마음</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* 학급 관리 섹션 */}
          <ClassroomManagement teacherName={teacherName} school={school} setSchool={setSchool} grade={grade} setGrade={setGrade} classNumber={classNumber} setClassNumber={setClassNumber} studentInputs={studentInputs} setStudentInputs={setStudentInputs} classroomId={classroomId} hasExistingClassroom={hasExistingClassroom} loadClassroomData={loadClassroomData} />

          {/* 설문 관리 섹션 */}
          <SurveyManagement school={school} grade={grade} classNumber={classNumber} teacherName={teacherName} />
        </div>
      </div>
    </div>;
};
