-- Create profiles table to link authenticated users to teacher names
...
-- Add user_id column to classrooms to establish ownership
...
-- Update existing classrooms to set user_id based on teacher_name
-- This will need to be done manually for existing data, but we'll set a default policy
...
-- Keep questions public for now since they're generic survey questions
-- But restrict relationship_responses to only responses involving students the user has access to
